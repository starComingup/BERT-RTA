...
from transformers import BertLayer, BertPreTrainedModel, BertModel
...
class BertModel4Mix(BertPreTrainedModel):
    ...
    def forward(self, input_ids,  input_ids2=None, l=None, mix_layer=1000, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None):
        ...
        if not parameters:
            raise ValueError("Model parameters are not initialized or loaded.")
...
class MixText(nn.Module):
    def __init__(self, num_labels=2, mix_option=False):
        ...
        if mix_option:
            self.bert = BertModel4Mix.from_pretrained('./dataroot/models/bert-base-uncased')
        else:
            self.bert = BertModel.from_pretrained('./dataroot/models/bert-base-uncased')
...
if __name__ == "__main__":
    model = MixText(num_labels=2, mix_option=True)
    input_ids = torch.tensor([[101, 1023, 102]]).to(torch.long)
